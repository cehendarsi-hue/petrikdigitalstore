...
function normalizeAppearance(source = {}) {
  const fallback = defaultData.appearance;
  const defaultMode = source.defaultMode === "light" ? "light" : "dark";

  return {
    defaultMode: defaultMode || fallback.defaultMode,
    showToggle: typeof source.showToggle === "boolean" ? source.showToggle : fallback.showToggle
  };
}
